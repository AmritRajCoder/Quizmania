         ...
         firebase.auth().createUserWithEmailAndPassword(this.state.email, this.state.password)
            .then((user) => {
               try {
                  ...
                  const usersDbRef = firebase.app().database().ref('users/');//firebase from importing config. It will create users if not there

                  //.child() will go in a level or create one
                  usersDbRef
                     .child(userId)
                     .set({
                        userId,
